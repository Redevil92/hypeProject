...
function URLParser() {
    ...
    console.log(getQueryParams(ref));
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(device[0].Prezzo == 0) { //devices not implemented in the DB
                ...
                rightDesc = '<div class="col-sm-6"><div class="well text-center"><h1>Dati per questo dispositivo non disponibili. Consultare il link selezionato sulla pagina principale</h1></div></div></div>';
            }
                ...
                //right column
                rightDesc = '<div class="col-sm-6"><h1>Descrizione</h1><p>'+device[0].InformazioniVeloci+'</p><div class="well text-center"><h3><b>Acquistalo a prezzo pieno</b></h3><h1 class="price">'+device[0].Prezzo+'€</h1><button type="button" class="btn btn-primary">Aggiungi al carrello</button></div><div class="well text-center"><h4>Scegli una delle offerte TIM e ricevi questo device a soli</h4><h1> class="price">'+device[0].PrezzoAbbonamento+'€/mese</h1><button type="button" class="btn btn-primary">Abbina all offerta</button></div></div>';
            }

             
            var technical ='';
            
            technical = '<div class="row"><ul class="nav nav-tabs bottomtabs"><li class="active"><a data-toggle="tab" href="#home">Scheda Tecnica</a></li><li><a data-toggle="tab" href="#menu1">Consegna</a></li><li><a data-toggle="tab" href="#menu2">Recesso e garanzia</a></li></ul><div class="tab-content"><div id="home" class="tab-pane fade in active"><pre style="margin-left:10px">'+device[0].Descrizione+'</pre></div><div id="menu1" class="tab-pane fade"><h3>Boh</h3><p>Some content in menu 1.</p></div><div id="menu2" class="tab-pane fade"><h3>Altro Boh</h3><p>Some content in menu 2.</p></div></div></div>';
